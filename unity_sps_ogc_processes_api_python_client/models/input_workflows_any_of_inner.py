...
INPUTWORKFLOWSANYOFINNER_ANY_OF_SCHEMAS = [
    "Bbox1",
    "InputCollection",
    "InputParameterized",
    "InputProcess",
...
class InputWorkflowsAnyOfInner(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = InputWorkflowsAnyOfInner.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in InputWorkflowsAnyOfInner with anyOf schemas: Bbox1, InputCollection, InputParameterized, InputProcess, Link, List[object], QualifiedInputValueWorkflows, bool, float, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into InputWorkflowsAnyOfInner with anyOf schemas: Bbox1, InputCollection, InputParameterized, InputProcess, Link, List[object], QualifiedInputValueWorkflows, bool, float, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
