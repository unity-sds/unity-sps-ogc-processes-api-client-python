...
class ActualInstance1(BaseModel):
    """
    ActualInstance1
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        instance = ActualInstance1.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in ActualInstance1 with anyOf schemas: Bbox1, InputCollection, InputParameterized, InputProcess, List[object], bool, float, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ActualInstance1 with anyOf schemas: Bbox1, InputCollection, InputParameterized, InputProcess, List[object], bool, float, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
