...
class ActualInstance(BaseModel):
    """
    ActualInstance
    """

    # data type: InputValueNoObjectWorkflows
    anyof_schema_1_validator: Optional[InputValueNoObjectWorkflows] = None
    # data type: Link
    anyof_schema_2_validator: Optional[Link] = None
    # data type: QualifiedInputValueWorkflows
    anyof_schema_3_validator: Optional[QualifiedInputValueWorkflows] = None
    if TYPE_CHECKING:
        actual_instance: Optional[
            Union[InputValueNoObjectWorkflows, Link, QualifiedInputValueWorkflows]
        ] = None
    else:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        ActualInstance.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in ActualInstance with anyOf schemas: InputValueNoObjectWorkflows, Link, QualifiedInputValueWorkflows. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ActualInstance with anyOf schemas: InputValueNoObjectWorkflows, Link, QualifiedInputValueWorkflows. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
