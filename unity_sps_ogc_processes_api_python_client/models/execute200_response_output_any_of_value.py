...
EXECUTE200RESPONSEOUTPUTANYOFVALUE_ANY_OF_SCHEMAS = [
    "Bbox",
    "Link",
    "List[object]",
    "QualifiedInputValueOutput",
...
class Execute200ResponseOutputAnyOfValue(BaseModel):
    """
    ...
    # data type: QualifiedInputValueOutput
    anyof_schema_8_validator: Optional[QualifiedInputValueOutput] = None
    if TYPE_CHECKING:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = Execute200ResponseOutputAnyOfValue.model_construct()
        error_messages = []
        ...
        # validate data type: QualifiedInputValueOutput
        if not isinstance(v, QualifiedInputValueOutput):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `QualifiedInputValueOutput`"
            )
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in Execute200ResponseOutputAnyOfValue with anyOf schemas: Bbox, Link, List[object], QualifiedInputValueOutput, bool, float, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_8_validator: Optional[QualifiedInputValueOutput] = None
        try:
            instance.actual_instance = QualifiedInputValueOutput.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into Execute200ResponseOutputAnyOfValue with anyOf schemas: Bbox, Link, List[object], QualifiedInputValueOutput, bool, float, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
