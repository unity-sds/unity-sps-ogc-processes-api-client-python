...
METADATAOUTPUT_ANY_OF_SCHEMAS = ["MetadataOneOf", "MetadataOneOf1"]
...
class MetadataOutput(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        MetadataOutput.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in MetadataOutput with anyOf schemas: MetadataOneOf, MetadataOneOf1. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into MetadataOutput with anyOf schemas: MetadataOneOf, MetadataOneOf1. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
