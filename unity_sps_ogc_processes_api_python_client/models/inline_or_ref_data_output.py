...
INLINEORREFDATAOUTPUT_ANY_OF_SCHEMAS = [
    "Bbox",
    "Link",
    "List[object]",
    "QualifiedInputValueOutput",
...
class InlineOrRefDataOutput(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = InlineOrRefDataOutput.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in InlineOrRefDataOutput with anyOf schemas: Bbox, Link, List[object], QualifiedInputValueOutput, bool, float, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into InlineOrRefDataOutput with anyOf schemas: Bbox, Link, List[object], QualifiedInputValueOutput, bool, float, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
