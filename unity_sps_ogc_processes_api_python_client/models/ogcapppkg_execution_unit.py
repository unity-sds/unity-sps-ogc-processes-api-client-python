...
OGCAPPPKGEXECUTIONUNIT_ANY_OF_SCHEMAS = [
    "ExecutionUnit",
    "Link",
    "List[OgcapppkgExecutionUnitAnyOfInner]",
    "QualifiedInputValueInput",
]
...
class OgcapppkgExecutionUnit(BaseModel):
    ...
    # data type: List[OgcapppkgExecutionUnitAnyOfInner]
    anyof_schema_4_validator: Optional[List[OgcapppkgExecutionUnitAnyOfInner]] = None
    if TYPE_CHECKING:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        # validate data type: List[OgcapppkgExecutionUnitAnyOfInner]
        try:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in OgcapppkgExecutionUnit with anyOf schemas: ExecutionUnit, Link, List[OgcapppkgExecutionUnitAnyOfInner], QualifiedInputValueInput. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into List[OgcapppkgExecutionUnitAnyOfInner]
        try:
            # validation
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into OgcapppkgExecutionUnit with anyOf schemas: ExecutionUnit, Link, List[OgcapppkgExecutionUnitAnyOfInner], QualifiedInputValueInput. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
